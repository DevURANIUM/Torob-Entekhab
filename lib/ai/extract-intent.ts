...
const brandWords = [
  ...
  "ردمی|redmi",
  "پوکو|poco",
  "گوگل|پیکسل|pixel|google",
  "وان پلاس|وانپلاس|oneplus",
];
export function extractIntent(
  ...
  if (/مامان|مادر|پدر|بابا/.test(q)) {
    ...
    result.assumptions = [
      "برای والدین، باتری، خوانایی و دوام نرم‌افزاری را مهم و بازی را کم‌اهمیت فرض کردیم؛ سن یا توانایی را فرض نکردیم. از ویرایش می‌توانی تغییر بدهی.",
    ];
    result.priorities.battery ??= 4;
    result.priorities.longevity ??= 6;
    result.priorities.performance ??= 0.5;
    result.priorities.display ??= 3;
    result.priorities.simplicity ??= 3;
  }
  ...
  if (/بازی|پابجی|گیم/.test(q) && result.priorities.performance !== 0.25)
    add("gaming");
  if (
    /(?:بازی|گیم).*(?:مهم نیست|بی خیال|بیخیال|فراموش|نمی کنم)|بی خیال.*(?:بازی|گیم)/.test(
      q,
    )
  ) {
    result.useCases = result.useCases.filter((x) => x !== "gaming");
    result.priorities.performance = 0.25;
  }
  if (/سالمند|کاربری ساده/.test(q)) add("elderly");
  if (/سفر|مسافرت/.test(q)) add("travel");
  if (/کوچک|کوچیک|جمع و جور/.test(q)) {
    add("compact");
    result.priorities.portability = 6;
  }
  ...
  if (storage && !/رم\s*$/.test(q.slice(0, storage.index)))
    result.constraints.minStorage = Number(storage[1]);
  ...
  if (/ارزون تر|ارزان تر|کمتر خرج|صرفه جویی/.test(q)) {
    result.action = "save";
    if (previous?.priorities.value !== undefined)
      result.priorities.value = previous.priorities.value;
    else delete result.priorities.value;
  }
  if (/دو.*اول.*مقایسه|مقایسه.*دو.*اول|اولی.*دومی/.test(q))
    result.action = "compare_first_two";
  if (/حافظه.*بیشتر/.test(q) && !storage)
    result.constraints.minStorage = Math.min(
      1024,
      (result.constraints.minStorage ?? 128) * 2,
    );
  if (/شارژ سریع/.test(q)) result.priorities.charging = 5;
  if (/نو باشه|فقط نو|دست دوم نه/.test(q)) result.constraints.newOnly = true;
  const charge = q.match(/(?:شارژ|حداقل)\s*(\d+)\s*وات/);
  if (charge) result.constraints.minCharging = Number(charge[1]);
  result.warnings = result.brands.some((b) => result.excludedBrands.includes(b))
    ? ["برند انتخابی و حذف‌شده متناقض‌اند."]
    : [];
  if (result.budget?.kind === "soft")
    result.assumptions = Array.from(
      new Set([
        ...result.assumptions,
        "بودجه حدودی است؛ گزینه بالاتر فقط جداگانه و با بهبود معنادار نمایش داده می‌شود.",
      ]),
    );
  if (/فریم|fps/.test(q))
    result.warnings.push(
      "فریم بازی تضمین نمی‌شود؛ بنچمارک یا تست حرارتی این بازی ثبت نشده.",
    );
  if (/شب|تبلیغات نرم افزاری/.test(q))
    result.warnings.push(
      "کیفیت عکاسی شب یا تجربه تبلیغات نرم‌افزار در داده‌ها اندازه‌گیری نشده است.",
    );
  const flexible = q.match(/حدود\s*(\d+).*تا\s*(\d+)/);
  if (flexible && Number(flexible[2]) > Number(flexible[1])) {
    const target = Number(flexible[1]) * 1e6;
    result.budget = {
      max: target,
      target,
      kind: "soft",
      flexibility: Math.min(0.3, Number(flexible[2]) / Number(flexible[1]) - 1),
    };
  }
